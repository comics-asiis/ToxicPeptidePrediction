...
def Load4modelsAndPredict(input_fastapath, ul_seqlen=50, rnn_cutoff=0.5, output_dir=None):
    ...
    pred_label_svc = predict_using_srg(pred_seqDi, None, encode_profile=True, profile_type=0, model=model_svc,
                                            scaler=scaler_svc, max_seq_len=ul_seqlen)
    pred_label_rf = predict_using_srg(pred_seqDi, None, encode_profile=True, profile_type=0, model=model_rf,
                                           scaler=scaler_rf, max_seq_len=ul_seqlen)
    pred_label_gbc = predict_using_srg(pred_seqDi, None, encode_profile=True, profile_type=0, model=model_gbc,
    ...
    pred_label_rnn = predict_using_rnn(pred_seqDi, max_seq_len=ul_seqlen, cutoff=rnn_cutoff)
...
def predict_using_rnn(pred_seqDi, given_features=None, profile_type=0, max_seq_len=50, cutoff=0.5):
    ...
    if profile_type == 0:
        y_pred, x_pred = extractfeature.ConstructSequenceProfile_GivenPropertyFeatures(initial_df_sequence_pred,
                                                                                       ...
                                                                                       reshape=True,
                                                                                       with_label=False)
    model = create_rnn_model(x_pred,num_unit=64)
...
def predict_using_srg(pred_seqDi, given_features, encode_profile, profile_type=0, model=None, scaler=None, max_seq_len=50):
...
def create_rnn_model(input_feature, num_unit=64):
